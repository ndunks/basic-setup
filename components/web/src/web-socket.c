...
static void send_ping_close(ws_cli_conn_t *cli, int threshold, int lock)
{
	...
	pthread_mutex_lock(&cli->mtx_ping);
		...
		if ((cli->current_ping_id - cli->last_pong_id) > threshold){
            DEBUG( "Closing: due too pong to long");
 ...
 * for completeness.
 */
static int do_handshake(ws_cli_conn_t *wsclient, char *http_header)
{
    ...
    /* Get response. */
    if (get_handshake_response(http_header, &response) < 0)
    {
        DEBUG("Cannot get handshake response");
        return (-1);
    }
    ...
    /* Send handshake. */
    if (SEND(wsclient, response, strlen(response)) < 0)
    {
        free(response);
...
static inline int next_byte(struct ws_frame_data *wfd)
{
    ...
    if (wfd->cur_pos == 0 || wfd->cur_pos == wfd->amt_read)
    {
        ...
        {
            wfd->error = 1;
            DEBUG("An error has occurred while trying to read next byte %0x\n", n);
            return (-1);
        }
 ...
 * for completeness.
 */
static void *ws_establishconnection(ws_cli_conn_t *client)
{
    ...
    while (next_frame(&wfd) >= 0)
    {
        ...
        if ((wfd.frame_type == WS_FR_OP_TXT || wfd.frame_type == WS_FR_OP_BIN) &&
            !wfd.error)
        {
            ws_onmessage(client, wfd.msg, wfd.frame_size, wfd.frame_type);
        }
...
esp_err_t ws_accept(int sock, char *http_header, ssize_t http_header_len)
{
    ...
    {
        close_client(wsclient, false);
        return ESP_FAIL;
    }

    /* Client socket added to socks list ? */
    if (pthread_create(&client_thread, NULL, ws_establishconnection, wsclient))
    {
        close_client(wsclient, false);
